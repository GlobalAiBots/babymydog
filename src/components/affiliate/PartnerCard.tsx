...
export default function PartnerCard({ partner, tagline, description, primary, secondary }: Props) {
  ...
  return (
    <div
      className="bg-white rounded-2xl p-6 border border-[#C4704B]/15"
    ...
    >
      <div className="flex items-start gap-4 mb-4">
        ...
        <div>
          <h3 className="font-bold text-[#1A1A1A] text-lg leading-snug">{partner.name}</h3>
          <p className="text-xs uppercase tracking-wider text-[#C4704B] font-bold mt-0.5">{tagline}</p>
        </div>
      </div>
      <p className="text-sm text-[#1A1A1A]/70 leading-relaxed mb-4">{description}</p>
      <div className="flex flex-wrap gap-2">
        ...
        {secondary && (
          <a
            ...
            className="inline-flex items-center gap-1.5 text-[#C4704B] border border-[#C4704B]/30 hover:bg-[#C4704B]/5 text-sm font-bold px-4 py-2 rounded-lg transition-colors"
          >
            {secondary.label}
          </a>
        )}
